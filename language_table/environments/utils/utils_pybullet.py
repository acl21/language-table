...
def load_urdf(pybullet_client, file_path, *args, **kwargs):
    ...
    try:
        ...
        raise FileNotFoundError(
            "Cannot load the URDF file {}".format(file_path)
        ) from exc
...
def add_visual_circle(client, center=(0, 0, 0), radius=0.1, rgba=(0.5, 0.5, 0.5, 0.5)):
    """Add a circle to bullet scene (visual only, no physics).

    Args:
      client: pybullet client (or pybullet library handle).
      center: Center of circle.
      radius: Circle radius.
      rgba: rgba color of circle.

    Returns:
      Unique integer bullet id of constructed object.
    """
    vis_obj_id = client.createVisualShape(
        client.GEOM_CYLINDER, radius=radius, length=0.001, rgbaColor=rgba
    )
    obj_id = client.createMultiBody(
        baseCollisionShapeIndex=-1, baseVisualShapeIndex=vis_obj_id, basePosition=center
    )
...
def get_workspace(env):
    (workspace_origin, workspace_quat) = (
...
def reset_camera_pose(env, view_type):
    ...
    if view_type == "POLICY":
        ...
        # TODO(tompson): Request Erwin add method to set view and projection matrix
        # of the debug visualizer directly (because using yaw and pitch is
        # inconsistent).
