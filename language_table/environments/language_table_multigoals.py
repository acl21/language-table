...
from language_table.environments.utils.utils_pybullet import add_visual_circle
...
rgba_dict = {
    RED_GOAL: (1, 0.4, 0.4, 0.5),
    GREEN_GOAL: (0.4, 1, 0.4, 0.5),
    BLUE_GOAL: (0.4, 0.4, 1, 0.5),
    YELLOW_GOAL: (1, 1, 0.4, 0.5),
    OUT_OF_BOUNDS: (0.0, 0.0, 0.0, 0.0),
}
...
class LanguageTable(gym.Env):
    ...
    def __init__(
        ...
        if reward_factory is not None:
            ...
            if goal_color is not None:
                self._reward_calculator.set_goal_from_text(goal_color)
            else:
    ...
    def reset(self, reset_poses=True):
        ...
        if self._reward_calculator is not None and self._show_goals:
            goal, goal_radius = self._reward_calculator.get_goal_region()
        ...
        if self._robot.end_effector:
            state["robot_end_effectors"].append(
                ObjState.get_bullet_state(
                    self._pybullet_client, self._robot.end_effector
                )
            )
    ...
    def set_pybullet_state(self, state):
        ...
        if "start_block" in state:
            self._start_block = self.decode_instruction(np.array(state["start_block"]))
    ...
    def _compute_state(self, request_task_update=True):
        ...
        for block in self._get_urdf_paths().keys():
            ...
            block_position_and_orientation = (
                self._pybullet_client.getBasePositionAndOrientation(block_id)
            )
            poses[block] = block_position_and_orientation

        e_target_trans = np.array(
        ...
        for k in self._get_urdf_paths().keys():
            ...
            mask = 1.0 if k in self._blocks_on_table else 0.0
        ...
        # this check.
        if (
            hasattr(self._reward_calculator, "get_current_task_info")
            and request_task_update
        ):
        ...
        obs.update(
            collections.OrderedDict(
                effector_translation=np.array(
                    effector_pose.translation[0:2], np.float32
                ),
    ...
    def _yaw_from_pose(self, pose):
        return np.array(
            [self._pybullet_client.getEulerFromQuaternion(pose[1])[-1]], np.float32
        )
    ...
    def _render_camera(self, image_size, return_seg_mask=False):
        ...
        viewm, projm, _, _, _ = self.calc_camera_params(
            image_size
    ...
    def _setup_pybullet_scene(self):
        ...
        if self._show_goals:
            ...
            self._goal_circle[BLUE_GOAL] = add_visual_circle(
                self._pybullet_client,
                center=(0, 0, 0),
                radius=0.02,
                rgba=rgba_dict[BLUE_GOAL],
            )
            self._goal_circle[YELLOW_GOAL] = add_visual_circle(
                self._pybullet_client,
                center=(0, 0, 0),
                radius=0.02,
                rgba=rgba_dict[YELLOW_GOAL],
            )
    ...
    def _set_task_info(self, info):
        ...
        if isinstance(info, task_info.Block2BlockTaskInfo):
            ...
            self._target_absolute_location = info.location
            self._target_relative_location = None
        elif isinstance(info, task_info.Block2MultiColoursTaskInfo):
            # Get the info parameterizing "block2multicolours" tasks.
            instruction_str = info.instruction
            self._start_block = info.block
            self._oracle_target_block = None
            self._oracle_target_translation = info.target_translation
        ...
        if (
            ...
            raise ValueError(
                "We are not providing the oracle with a target location "
    ...
    def _reset_poses_randomly(self, blocks_on_table):
        ...
        for block_id in visible_block_ids:
            far_translation = np.array([5.0, 5.0, 0.0])
            far_rotation = transform.Rotation.from_rotvec([np.pi / 2, 0, 0])
            self._pybullet_client.resetBasePositionAndOrientation(
                block_id, far_translation.tolist(), far_rotation.as_quat().tolist()
            )
        # Step simulation to move blocks out of the way.
        self._step_simulation_to_stabilize()

        xmin, ymin = (
            constants.X_MIN + constants.WORKSPACE_BOUNDS_BUFFER,
            constants.Y_MIN + constants.WORKSPACE_BOUNDS_BUFFER,
        )
        xmax, ymax = (
            constants.X_MAX - constants.WORKSPACE_BOUNDS_BUFFER,
        ...
        visible_block_ids = [self._block_to_pybullet_id[i] for i in blocks_on_table]
        invisible_block_ids = [i for i in self._block_ids if i not in visible_block_ids]

        # Cast the "invisible" blocks way off the board.
        for block_id in invisible_block_ids:
            far_translation = np.array([5.0, 5.0, 0])
            far_rotation = transform.Rotation.from_rotvec([np.pi / 2, 0, 0])
            self._pybullet_client.resetBasePositionAndOrientation(
                block_id, far_translation.tolist(), far_rotation.as_quat().tolist()
            )
    ...
    def _construct_invisible_walls(self):
        ...
        if visible:
            ...
            vis_shape_id = pybullet.createVisualShape(
                shapeType=pybullet.GEOM_SPHERE, radius=1e-6, rgbaColor=[1, 1, 1, 0.0]
            )
        ...
        box = pybullet.createCollisionShape(
            pybullet.GEOM_BOX, halfExtents=[distance_x, extent_width, extent_height]
        )
        ...
        # TOP
        box = pybullet.createCollisionShape(
            pybullet.GEOM_BOX, halfExtents=[extent_width, distance_y, extent_height]
        )
...
def add_debug_info_to_image(
    ...
    whitespace = np.ones([whitespace_height, image.shape[1], 3], dtype=np.uint8) * 255
